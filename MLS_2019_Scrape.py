...
PAGE_url = FBREF + '/en/comps/22/2798/2019-Major-League-Soccer-Stats'  # ENSURE LINK IS FOR RIGHT YEAR!
...
hyperlinkTEXTS = FULL_beautifulsoup.find_all('tbody')[4].find_all(href = re.compile('squads'))  # THEN 5th TABLE after 2 * OVERALL, 2 * HOME/AWAY. # '(href = re.compile('squads'))' narrows JUST LIKE '.find('a')'.
# print(len(hyperlinkTEXTS))  # 24, as it SHOULD BE for 2019 SEASON.
...
for hyperlinkTEXT in hyperlinkTEXTS:
    ...
    if SLEEPER >= 0.723:
        SLEEPING = random.uniform(random.randint(16, 20), random.randint(28, 31))
        print('FIRST STOP:', SLEEPER, '(%g seconds)' % SLEEPING)
        time.sleep(SLEEPING)  # DOUBLE RANDOM NUMBER.
    elif SLEEPER >= 0.2432:
        SLEEPING = random.uniform(random.randint(5, 7), random.randint(12, 14))
        print('FIRST STOP:', SLEEPER, '(%g seconds)' % SLEEPING)
        time.sleep(SLEEPING)  # DOUBLE RANDOM NUMBER.
    else:
    ...
    SLEEPER = random.random()
    if SLEEPER >= 0.1132:
        SLEEPING = random.uniform(random.randint(9, 11), random.randint(12, 15))
    ...
    SLEEPER = random.random()
    if SLEEPER >= 0.29342:
        SLEEPING = random.uniform(random.randint(9, 10), random.randint(20, 45))
    ...
    # FOURTH RANDOM SLEEP.
    SLEEPER = random.random()
    if SLEEPER >= 0.7594381:
        SLEEPING = random.uniform(random.randint(6, 9), random.randint(20, 25))
...
# PRINTING/FINAL POLISHES.
# DATAFRAME = DATAFRAME.reset_index(drop = True)  # NOTE: RETURNS a NEW OBJECT. NOT NEEDED?
print(DATAFRAME)
print()
DATAFRAME.to_csv('MLS_2019.csv', index = False)
