...
for hyperlinkTEXT in hyperlinkTEXTS:
    ...
    elif SLEEPER >= 0.24322:
        SLEEPING = random.uniform(random.randint(9, 11), random.randint(13, 15))
        print('FIRST STOP:', SLEEPER, '(%g seconds)' % SLEEPING)
        time.sleep(SLEEPING)  # DOUBLE RANDOM NUMBER.
    else:
    ...
    SLEEPER = random.random()
    if SLEEPER >= 0.121329:
        SLEEPING = random.uniform(random.randint(9, 12), random.randint(15, 21))
    ...
    SLEEPER = random.random()
    if SLEEPER >= 0.279342:
        SLEEPING = random.uniform(random.randint(9, 10), random.randint(22, 44))
...
print(DATAFRAME)
print()
DATAFRAME.to_csv('MLS_2018.csv', index = False)
